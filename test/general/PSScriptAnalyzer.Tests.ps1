$CommandPath = @(
	"C:\Git\Private\psoauth2\src\private"
	"C:\Git\Private\psoauth2\src\public"
)
...
Describe 'Invoking PSScriptAnalyzer against commandbase' {
	...
	foreach ($file in $commandFiles)
	{
		Context "Analyzing $($file.BaseName)" {
			$analysis = Invoke-ScriptAnalyzer -Path $file.FullName -ExcludeRule PSAvoidTrailingWhitespace, PSShouldProcess, PSUseShouldProcessForStateChangingFunctions, PSAvoidGlobalVars, PSAvoidUsingEmptyCatchBlock, PSAvoidUsingWriteHost, PSUseApprovedVerbs
